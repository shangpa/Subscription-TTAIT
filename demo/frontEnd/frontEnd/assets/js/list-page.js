...
const typeOptions = [
  ...
  { value: "공공임대", label: "공공임대", icon: '<rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/>' },
  ...
  return `
    <div class="pagination">
      ${pages
        .map(
          (value) => `
            <button class="page-btn ${String(value) === String(page) ? "is-active" : ""}" type="button" data-page="${value}">
              ${value}
            </button>
          `
        )
  ...
  root.innerHTML = `
    ...
    <section class="container page-shell list-page__body">
      <div class="list-page__filters">
        <div class="chip-row">
          <button class="chip ${currentState.region === "경기" ? "is-active" : ""}" type="button" data-region="경기">경기</button>
...
function bindFilters() {
  root.querySelectorAll("[data-type]").forEach((button) => {
    button.addEventListener("click", () => setQueryParams({ type: button.dataset.type, page: "1" }));
  });
  ...
  root.querySelectorAll("[data-sort]").forEach((button) => {
    button.addEventListener("click", () => setQueryParams({ sort: button.dataset.sort, page: "1" }));
  });
  ...
  root.querySelectorAll("[data-region]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      setQueryParams({ region: next, page: "1" });
    });
  });

  root.querySelectorAll("[data-page]").forEach((button) => {
    button.addEventListener("click", () => setQueryParams({ page: button.dataset.page }));
  });
}
