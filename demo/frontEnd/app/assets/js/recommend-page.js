  ...
  root.innerHTML = `
    <section class="container page-shell recommend-page__body">
      <div class="recommend-hero">
        ...
        <p>현재 입력된 선호 조건인 ${result.profile.preferredRegion} ${result.profile.preferredDistrict}, ${result.profile.preferredSupplyType}, 최대 보증금 ${formatCurrency(result.profile.maxDeposit)}를 기준으로 우선순위를 계산했습니다.</p>
        <div class="recommend-summary">
          ...
          <span class="summary-pill">관심 유형 · ${result.profile.categories.join(", ")}</span>
      ...
      <section class="recommend-page__section">
        <div class="chip-row" style="margin-bottom:20px;">
          ...
          <span class="chip">출근 거리 고려</span>
          <span class="chip">예산 적합도</span>
          <span class="result-count">총 <strong>${result.items.length}</strong>개의 추천 공고</span>
        </div>
        <div class="listing-grid">
          ${result.items.map((item) => `<div>${renderListingCard(item)}<div style="padding:8px 12px 0;"><p class="meta-line" style="color:var(--rausch);font-weight:700;">추천 점수 ${item.score}점</p><p class="meta-line" style="margin-top:4px;line-height:1.6;">${item.recommendedReason}</p></div></div>`).join("")}
        </div>
      </section>
    </section>
  `;
