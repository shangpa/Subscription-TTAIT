import { fetchListingDetail } from "./api.js";
...
async function init() {
  const item = await fetchListingDetail(queryParam("id", "osan-segyo-21"));
  document.title = `집구해 - ${item.title}`;
    ...
    <section class="container page-shell detail-page__body">
      <div class="layout-grid detail-layout">
        <article>
          ...
          <section class="detail-section">
            <h2 class="detail-section__title">신청 일정</h2>
            <div class="timeline">
              ${item.schedule.map((step) => `<div class="timeline-item ${step.current ? "is-current" : ""}"><p class="timeline-date ${step.current ? "is-current" : ""}">${step.date}${step.badge ? `<span class="deadline-pill">${step.badge}</span>` : ""}</p><p class="timeline-title">${step.title}</p>${step.desc ? `<p class="timeline-desc">${step.desc}</p>` : ""}</div>`).join("")}
            </div>
          </section>
          <section class="detail-section">
            ...
            <div class="market-card">
              ...
              <div class="market-row"><span class="market-label">보증금</span><div class="market-compare"><span class="market-value-market">${item.market.depositMarket}</span><span>→</span><span class="market-value-public">${item.market.depositPublic}</span></div></div>
              ...
              <div class="market-row"><span class="market-label">월세</span><div class="market-compare"><span class="market-value-market">${item.market.rentMarket}</span><span>→</span><span class="market-value-public">${item.market.rentPublic}</span></div></div>
          ...
          <section class="detail-section">
            ...
            <div class="attachment-list">
              ${item.attachments.map((attachment) => `<a class="attachment-item" href="${attachment.href}"><span class="attachment-icon"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6zM13 3.5 18.5 9H13z"/></svg></span><span class="attachment-name">${attachment.label}</span><span class="attachment-type">${attachment.type}</span><span>↗</span></a>`).join("")}
            </div>
          </section>
          <section class="detail-section">
            ...
            <div class="info-panel">
              <p>${item.notes}</p>
              <a class="pill-btn" href="${item.applyUrl}" target="_blank" rel="noreferrer" style="margin-top:12px;padding:0;">LH 청약센터 바로가기</a>
            </div>
          </section>
        </article>
        <aside>
          <div class="sticky-card">
            ...
            <div class="deadline-info"><span>⏰</span><span>${item.deadlineLabel}</span></div>
            <div class="stack-actions">
              <a class="submit-btn" href="${item.applyUrl}" target="_blank" rel="noreferrer">신청하러 가기</a>
              <button class="save-btn" type="button">공고 저장하기</button>
            </div>
