...
import { rememberListingSnapshot, toggleSavedListing } from "./api.js";
...
const logo = `
  <a class="logo" href="./list.html">
    ...
    <span class="logo__text">집구해줘</span>
  </a>
`;
...
export function renderFooter(root) {
  ...
  root.innerHTML = `
    <div class="container site-footer__grid">
      ...
      <div class="site-footer__group"><h3>도움말</h3><p>공급유형 안내</p><p>자격 확인</p><p>자주 묻는 질문</p></div>
      <p class="site-footer__legal">© 2026 집구해줘 공공임대주택 정보 서비스</p>
    </div>
  `;
}
  ...
  return `
    <a class="listing-card" href="./detail.html?id=${item.id}">
      ...
      <div class="listing-card__body">
        ...
        <div class="tag-list">
          ${Array.isArray(item.tags) ? item.tags.map((tag) => `<span class="tag-chip ${tagToneClass(tag.tone)}">${tag.label}</span>`).join("") : ""}
        </div>
      </div>
    </a>
  `;
...
export function attachCardInteractions(root) {
  root.querySelectorAll("[data-heart]").forEach((button) => {
    button.addEventListener("click", async (event) => {
      ...
      const nextValue = await toggleSavedListing(button.dataset.heart);
      button.classList.toggle("is-saved", nextValue);
