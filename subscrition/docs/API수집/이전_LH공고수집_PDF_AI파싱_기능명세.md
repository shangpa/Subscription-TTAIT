# [이전] LH 공고 수집 및 PDF AI 파싱 기능 명세

> 현재 상태: 이 문서는 dedupe/admin 후보 선택 API 도입 전의 LH 수집 및 PDF AI 파싱 기능명세다. 현재 기준 문서는 `현행_LH공고수집_중복방지_관리자선택흐름_기능명세.md`를 따른다. 기존 `/api/admin/import/lh`는 유지되지만, 현재는 후보 수집/목록/선택 import/force reparse API와 fingerprint 기반 Gemini skip이 추가되었다.

## 기능 개요

LH 외부 API에서 공공임대 공고를 수집하고, 공고에 첨부된 PDF를 Gemini로 파싱하여 DB에 저장하는 기능.
...
공고 1건당 **2개의 독립된 트랜잭션**으로 처리됩니다. 트랜잭션 사이 외부 API 호출(LH 상세 API, PDF 다운로드, Gemini)이 수행되며, 각 트랜잭션은 실패해도 서로 영향을 주지 않습니다.
...
> **Gemini 실패** → TX2 내 raw 필드가 null로 저장되지만 공고 자체는 정상 저장됩니다.
...
Gemini가 파싱한 자격조건을 정형화해서 저장. 관리자 검수 상태도 이 테이블이 관리함.
...
| `PDF_AI_JSON` | TX2 (`upsertLhDetail`) | Gemini 파싱 결과 JSON |
...
| Gemini API 실패 (복구 가능한 JSON 파싱 오류) | warn 로그, text fallback 또는 raw 필드 null, 공고는 정상 저장 |
| Gemini API 실패 (그 외) | error 로그, raw 필드 null, 공고는 정상 저장 |
...
2. LH 상세 API 재호출
   ↓
3. PDF URL 재추출 → PDF 재다운로드 → Gemini 재파싱
   ↓
...
## 이전 OpenAI 기반 테스트 결과 (2026-04-23)

> 아래 기록은 현재 Gemini 기반 구현으로 전환되기 전 OpenAI 기반 수집/파싱 테스트 기록이다. 현행 검증 상태는 `중간발표_이후/현행_LH공고수집_단위데이터구조개편_작업기록.md`를 기준으로 본다.
