# [이전] LH 공고 수집 트랜잭션 흐름 설명

> 현재 상태: 이 문서는 모든 LH 공고가 상세/PDF/Gemini 경로로 바로 진행되던 이전 흐름 설명이다. 현재는 fingerprint dedupe로 변경 없는 공고의 Gemini 호출을 생략하고, 관리자 후보 수집 단계에서는 Gemini를 호출하지 않는다. 현재 기준은 `현행_LH공고수집_중복방지_관리자선택흐름_기능명세.md`를 따른다.

> `현행_LH공고수집_PDF_AI파싱_기능명세.md`의 처리 흐름 섹션에 대한 심화 설명.
...
processLhItem(item)                  ← NoticeImportOrchestrator
  ...
  ├─ (트랜잭션 없음) pdfParsingService.parse(pdfUrl)      ← PDF 다운로드 + Gemini 파싱
  │
...
```
[문제 시나리오]
트랜잭션 안에서 Gemini 호출 → 비용 발생
→ 이후 DB 저장 실패 → 트랜잭션 롤백
→ DB는 롤백됐지만 Gemini 비용은 이미 청구됨
```
...
| TX1 성공 + Gemini 실패 | 공고는 저장됨, raw 필드 null |
| TX1 성공 + TX2 실패 | 공고는 저장됨, 상세/자격조건/카테고리만 미저장 |
| TX1 자체 실패 | 해당 공고 `failed` 카운트, 나머지 공고 계속 처리 |

이 구조 덕분에 **REIMPORT 기능**도 가능함.
TX1에서 `LH_ITEM_JSON`(원본 JSON)을 저장해두기 때문에, 나중에 이를 복원해 Gemini 파싱을 재실행할 수 있음.
...
| Gemini 파싱 | `external/pdf/PdfParsingService.java`, `external/ai/GeminiClient.java` |
